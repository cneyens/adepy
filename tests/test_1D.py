...
def test_finite1_shape():
    x = [5.0, 6.0, 7.0]
    c = finite1(1.0, x, 5, v=0.6, D=0.6, L=12)
    assert c.shape == (len(x),)   
    
    t = [10, 15, 20, 25]
    c = finite1(1.0, 2.5, t, v=0.6, D=0.6, L=12)
    assert c.shape == (len(t),)   

def test_finite3():

    c = finite3(1, [2.5, 5, 7.5], 5, v=0.6, D=0.6, L=12)
    np.testing.assert_array_almost_equal(c, np.array([0.55821, 0.17878, 0.02525]), decimal=5)

def test_finite3_shape():
    x = [5.0, 6.0, 7.0]
    c = finite3(1.0, x, 5, v=0.6, D=0.6, L=12)
    
    assert c.shape == (len(x),)   
    
    t = [10, 15, 20, 25]
    c = finite3(1.0, 2.5, t, v=0.6, D=0.6, L=12)
