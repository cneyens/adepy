...
def plume2(m0, x, y, t, v, n, al, ah, xc, yc, Dm=0.0, lamb=0.0, R=1.0):
    """Compute the fate of a 2D plume in an infinite aquifer with uniform background flow.

    The two-dimensional advection-dispersion equation is solved for concentration at specified `x` and `y location(s) and
    output time(s) `t`. An infinite system with uniform background flow in the x-direction contains an initial mass `m0`
    at locations `xc-yc`, which is subjected to advection, dispersion and 1st-order decay. It is assumed no source terms are present.

    If initial concentrations `c0` on a grid are available, e.g. from earlier results, the initial mass `m0` can be computed
    by multiplying `c0` by the x and y grid spacing and the porosity `n`.

    The solution is found by treating the initial mass values as individual instantaneous pulse sources with `pulse2()` and summing the results.


    Parameters
    ----------
    m0 : 1D or 2D array of floats
        Initial mass of the plume at locations `xc-yc` [M].
    x : float or 1D or 2D array of floats
        x-location(s) to compute output at [L].
    y : float or 1D or 2D array of floats
        y-location(s) to compute output at [L].
    ...
    if (x.size > 1 or y.size > 1) and t.size > 1:
        raise ValueError("Either x and y or t should be of length 1")
    ...
    for i, (mi, xi, yi) in enumerate(zip(m0, xc, yc)):
        c += pulse2(mi, x, y, t, v, n, al, ah, xi, yi, Dm, lamb, R)
